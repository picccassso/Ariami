...
@implementation AudioPlayer {
    ...
    // (Ariami fork) Optional graphic equalizer requested via darwinAudioEffects.
    JAEqualizer *_equalizer;
}
...
- (instancetype)initWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar playerId:(NSString*)idParam loadConfiguration:(NSDictionary *)loadConfiguration darwinAudioEffects:(NSArray *)darwinAudioEffects useLazyPreparation:(BOOL)useLazyPreparation {
    ...
    _equalizer = nil;
    if (darwinAudioEffects != (id)[NSNull null]) {
        for (NSDictionary *effect in darwinAudioEffects) {
            if ([@"DarwinEqualizer" isEqualToString:(NSString *)effect[@"type"]]) {
                _equalizer = [[JAEqualizer alloc] initWithFrequencies:(NSArray<NSNumber *> *)effect[@"frequencies"]
                                                                gains:(NSArray<NSNumber *> *)effect[@"gains"]
                                                              enabled:[effect[@"enabled"] boolValue]];
            }
        }
    }
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    @try {
        ...
        if ([@"load" isEqualToString:call.method]) {
            ...
            // (Ariami fork) The Dart DarwinEqualizer reuses this platform
            // method name because the platform interface defines no Darwin
            // variant; on iOS/macOS it can only ever mean our equalizer.
            if (_equalizer) {
                [_equalizer setGain:[request[@"gain"] doubleValue] forBand:[request[@"bandIndex"] intValue]];
            }
            result(@{});
        } else {
