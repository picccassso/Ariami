...
/// (Ariami fork) An iOS/macOS graphic equalizer implemented natively with an
/// MTAudioProcessingTap running peaking biquad filters over the player's
/// audio. Unlike [AndroidEqualizer], the band layout is chosen by the caller,
/// so [parameters] is available immediately without waiting for the platform
/// player to be created.
class DarwinEqualizer extends AudioEffect with DarwinAudioEffect {
  late final DarwinEqualizerParameters _parameters;

  DarwinEqualizer({
    List<double> frequencies = const [60.0, 230.0, 910.0, 3600.0, 14000.0],
    double minDecibels = -12.0,
    double maxDecibels = 12.0,
  }) {
    _parameters = DarwinEqualizerParameters._(
      equalizer: this,
  ...
  // Band gains and the enabled flag are carried into every platform init
  // request via _toMessage, so there is no state to restore in _activate.

  /// The parameter values of this equalizer.
  DarwinEqualizerParameters get parameters => _parameters;

  @override
  AudioEffectMessage _toMessage() => _DarwinEqualizerMessage(
        enabled: enabled,
        frequencies:
            _parameters.bands.map((band) => band.centerFrequency).toList(),
        gains: _parameters.bands.map((band) => band.gain).toList(),
      );
}
...
/// (Ariami fork) The parameter values of a [DarwinEqualizer].
class DarwinEqualizerParameters {
  ...
  DarwinEqualizerParameters._({
    required DarwinEqualizer equalizer,
    required this.minDecibels,
    required this.maxDecibels,
...
/// (Ariami fork) A frequency band within a [DarwinEqualizer].
class DarwinEqualizerBand {
  final DarwinEqualizer _equalizer;
  ...
  final _gainSubject = BehaviorSubject.seeded(0.0);
  ...
  Future<void> setGain(double gain) async {
    _gainSubject.add(gain);
    if (_equalizer._active) {
      // The platform interface defines no Darwin band-gain method, so the
      // Android method name is reused as a raw channel call; the darwin
      // implementation in this fork handles it for the equalizer.
      await (await _equalizer._player!._platform).androidEqualizerBandSetGain(
          AndroidEqualizerBandSetGainRequest(bandIndex: index, gain: gain));
    }
  }
}
