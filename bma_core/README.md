...
Platform-agnostic core library for BMA (Basic Music App). Contains shared business logic and services used by both bma_desktop and bma_cli.

## Overview

BMA Core is a pure Dart package (no Flutter dependencies) that provides music library management, HTTP server functionality, and data models for the BMA ecosystem. This package enables both GUI (desktop) and headless (CLI) server deployments to share the same core functionality.

## Architecture Role

- Shared by `bma_desktop` and `bma_cli` via path dependency
...
BMA Core is implemented in pure Dart without Flutter dependencies, enabling:
...
```dart
final httpServer = BmaHttpServer();
...
- `BmaHttpServer` - Prevents port conflicts
